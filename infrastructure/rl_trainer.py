...
class RL_Trainer(object):
    ...
    def train_agent(self, itr):
        # TODO: get this from hw1 or hw2
        all_logs = []
        num_iter_per_increment = self.params['warm_start_iter']//self.agent.critic.num_networks
        if self.params['warm_start_iter'] <= itr:
            if itr%num_iter_per_increment == 0:
                self.agent.critic.networks = [self.agent.critic.networks[-1], self.agent.critic.networks[-1]]
            self.agent.critic.switch = True
            # if we use the switch_based method
            for _ in range(self.params['num_agent_train_steps_per_iter']):
                ob_batch, ac_batch, re_batch, next_ob_batch, terminal_batch = self.agent.sample(self.params['train_batch_size'])
                train_log = self.agent.train(ob_batch, ac_batch, re_batch, next_ob_batch, terminal_batch, itr%num_iter_per_increment, None, None)
                all_logs.append(train_log)
        else:
            self.agent.critic.switch = False
            for i in range(self.agent.critic.num_networks):
                network_next = self.agent.critic.networks[i]
                if i != 0:
                    network_now = self.agent.critic.networks[i-1]
                else:
                    network_now = None
                for train_step in range(self.params['num_agent_train_steps_per_iter']):
                    ob_batch, ac_batch, re_batch, next_ob_batch, terminal_batch = self.agent.sample(self.params['train_batch_size'])
                    train_log = self.agent.train(ob_batch, ac_batch, re_batch, next_ob_batch, terminal_batch, i, network_now, network_next)
                    all_logs.append(train_log)
